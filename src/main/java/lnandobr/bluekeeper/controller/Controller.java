package lnandobr.bluekeeper.controller;
...
import lnandobr.bluekeeper.dao.DAOFactory;
...
import lnandobr.bluekeeper.util.StringUtils;
...
public class Controller {
    ...
    @FXML
    private LimitedTextField txtServico;
    @FXML
    private LimitedTextField txtLogin;
    @FXML
    private LimitedTextField txtSenha;
    @FXML
    ...
    private void initialize() {
        ...
        // Adiciona um listener para ser notificado quando o usuário limpa a pesquisa via teclado
        txtSearch.textProperty().addListener((event, oldValue, newValue) -> {
            if (newValue.isEmpty()) {
                // chama o botão de limpa
                clearSearch();
            }
        });
    ...
    private void loadData(boolean filter) {
        ...
        if (!filter) {
            ...
            // Carrega os dados com filtragem
    ...
    private boolean showConfirmDialog() {
        ...
        alert.setHeaderText("Ao Remover um registro, ele não poderá mais ser recuperado.");
        ...
        AtomicBoolean isYes = new AtomicBoolean(false);
